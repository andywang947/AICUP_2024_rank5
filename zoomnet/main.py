...
def parse_config():
    ...
    parser.add_argument("--config", default="./configs/zoomnet/sod_zoomnet.py", type=str)
...
def training(model, cfg) -> pipeline.ModelEma:
    ...
    print('cfg.path.final_state_net',cfg.path.final_state_net)#/mnt/disk1/roger/ZoomNet/output/ZoomNet_BS16_LR0.05_E2_H352_W352_OPMsgd_OPGMfinetune_SCf3_AMP_MS/pth/state_final.pth
    curr_iter = 0
    for curr_epoch in range(start_epoch, cfg.train.num_epochs):
        ...
        path1 = '/mnt/disk1/roger/ZoomNet/checkpoints/ZoomNet_epoch_%d.pth' % curr_epoch


        if curr_epoch % 10==0:
            io.save_weight(model=model, save_path=path1)
        if curr_iter >= cfg.train.num_iters:
            break
...
def main():
    ...
    cfg.tr_logger.record("End training...")
        ...
        #         }, os.path.join('./checkpoints',"model_{}.pth".format(curr_epoch+1)))
